...
fn test_depth_first_traversal() -> Result<()> {
    ...
    let _e1 = graph.add_link([v0, v1])?;
    let _e2 = graph.add_link([v1, v2])?;
    let _e3 = graph.add_link([v2, v3])?;
...
fn test_dft_branching_graph() -> Result<()> {
    ...
    let _e1 = graph.add_link([v0, v1])?;
    let _e2 = graph.add_link([v0, v2])?;
    let _e3 = graph.add_link([v0, v3])?;
    let _e4 = graph.add_link([v2, v4])?;
