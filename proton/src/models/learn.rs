...
pub struct RLearningUTM {
    ...
    q_values: HashMap<(usize, usize), HashMap<Rule, f64>>,
...
impl RLearningUTM {
                ...
                for next_state in 0..2 {
                    ...
                    for &write_symbol in &[0, 4, 7, 11] {
                        ...
                        for &direction in &[Direction::Left, Direction::Right] {
                            actions.push((next_state, write_symbol, direction));
                        }
                    }
                }
            }
    ...
    fn choose_action(&self, state: usize, symbol: usize) -> Rule {
        ...
        if let Some(q_map) = self.q_values.get(&(state, symbol)) {
            if !q_map.is_empty() {
    ...
    fn apply_rule(&mut self, rule: Rule) {
        let (next_state, write_symbol, direction) = rule;
    ...
    fn calculate_reward(&self, goal: &Goal) -> f64 {
        match goal {
            ...
            Goal::ReachState(target_state) => {
                ...
                if self.utm.state == *target_state { 1.0 } else { -0.1 }
            }
    ...
    pub fn train(&mut self, goal: Goal, episodes: usize) {
        ...
        for episode in 0..episodes {
            ...
            for _ in 0..self.max_steps {
                ...
                let max_next_q = self.q_values
                    .get(&(next_state, next_symbol))
                    .map(|q_map| {
                        q_map.values()
            ...
            if episode % 100 == 99 {
                ...
                println!("Episode {}: Reward = {:.2}, Epsilon = {:.3}", 
                         episode + 1, total_reward, self.epsilon);
            }
        }
        
        // Use the best rules found
        println!("Training complete. Best reward: {:.2}", best_reward);
        self.utm.transition_rules = best_rules;
    }
    
    /// Extract deterministic rules from Q-values
    pub fn extract_rules(&self) -> HashMap<(usize, usize), (usize, usize, Direction)> {
        let mut rules = HashMap::new();
        
        for (&(state, symbol), q_map) in &self.q_values {
            if !q_map.is_empty() {
                // Find action with max Q-value
                ...
                rules.insert((state, symbol), best_action);
            }
        }
    ...
    pub fn get_state(&self) -> (usize, Vec<usize>, usize, [usize; 3], TriadClass) {
        (
            self.utm.state,
            self.utm.tape.clone(),
            self.utm.position,
    ...
    pub fn print_rules(&self) {
        ...
        for ((state, symbol), (next_state, write_symbol, direction)) in &self.utm.transition_rules {
            println!("({}, {}) → ({}, {}, {:?})", 
