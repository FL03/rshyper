...
pub(crate) mod aliases {
    ...
    use rshyper_core::{HyperFacet, HyperNode};
    ...
    pub type BTreeFacet<E, K, Idx> = HyperFacet<E, BTreeSet<VertexId<Idx>>, K, Idx>;
    /// a type alias for a [`BTreeMap`] that uses a [`VertexId`] as a key for a [`HyperNode`]
    pub type NodeBMap<N, Idx> = BTreeMap<VertexId<Idx>, HyperNode<N, Idx>>;
