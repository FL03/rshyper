...
{
    /// the `links` of a hypergraph describe the relationships between edges and vertices
    pub(crate) links: HashMap<EdgeId<Idx>, HashSet<VertexId<Idx>>>,
    ...
    Idx: Eq + RawIndex + core::hash::Hash,
{
    ...
    /// returns an immutable reference to the _links_ of the hypergraph; here, a link is
    /// defined to be a mapping of edges to vertices essentially forming a topological space
    /// that enables the data-structure to be traversed, analyzed, and manipulated.
    pub const fn links(&self) -> &HashMap<EdgeId<Idx>, HashSet<VertexId<Idx>>> {
        &self.links
    }
    /// returns a mutable reference to the hyperedges
    pub const fn links_mut(&mut self) -> &mut HashMap<EdgeId<Idx>, HashSet<VertexId<Idx>>> {
        &mut self.links
    }
        ...
        Idx: Copy + core::ops::Add<Output = Idx> + num_traits::One,
    {
        ...
        self.links_mut().insert(eid, vset);
        Ok(eid)
    }
        ...
        Idx: Copy + core::ops::Add<Output = Idx> + num_traits::One,
    {
        ...
        self.links_mut().insert(new_edge, merged);
        Ok(new_edge)
    }
        ...
        Idx: Copy,
    {
        ...
        self.links().values().for_each(|vertices| {
            if vertices.contains(index) {
